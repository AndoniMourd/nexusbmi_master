%write out a table for cross-corr local max 
function cross_corr_table(corr_data,lag_idx)

DAYS = {'020217','020317','020617','020717','020817','020917','021017','021317','021417','021517'};
MOVE_BLOCKS = {'i','c','c','d','m','a','a','a','a','a'};

[num,txt,raw]= xlsread('daily_notes','MetaData','V4:V13');
med_time = txt;
[num,txt,raw]= xlsread('daily_notes','MetaData','W4:W13');
med_notes = raw;
f = figure;
t = uitable(f);
local_max = cell(10,1);
for i=1:10
    local_max{i} = max(corr_data{i}(find(lag_idx{i}==-40):find(lag_idx{i}==40)));
end
days = strcat(DAYS,MOVE_BLOCKS)';
d = [days(:),local_max(:),med_time,med_notes];
t.ColumnName = {' Day & Block ',' Cross-Corr Local Max ',' Medication Time       ',' Medication Notes         '};
t.Data = d;
t.Position = [35 115 475 200];
